...
|---|---|---|---|---|
| **A response section that omits `ai_backed` is invisible to the output-boundary check.** *Moved here 2026-09-26 from `ADDITION_SPEC_BATCH1.md` §"Tracked follow-up", at the frontend session's prompting — it asked whether this was still open and I would have answered from memory that the grounding work superseded it. It does not.* `output_boundary.py:330` `_ai_sections` collects only nodes where `node.get("ai_backed") is True`. `OUTPUT_BOUNDARY_GROUNDING_DESIGN.md` (`bada9d4`..`866176e`) changed what is checked *inside* a discovered section — declared `ai_fields` only, grounded per object, exact below 100 — and left discovery untouched. So forgetting one key removes the whole check for that section, silently. No test covers it: `tests/test_output_boundary.py` has nothing for an undeclared or marker-less section. It is the same shape as the citation defect closed in `CITATION_FORM_COVERAGE_DESIGN.md` — one artefact doing two jobs, here marking a section as AI-authored *and* making it visible to the checker, so omitting it fails open instead of loudly. | **Fail-open, second layer only.** The boundary is test-only, so nothing ungrounded reaches a user because of this: the inline guard in `ai_layer.py` still runs at every call site. What it means is that the backstop built to distrust those call sites can be bypassed by omission — the exact thing it exists to catch. | `output_boundary.py`. Owner: the numeric-guard work (this session's area). Decisions: project owner. | Measure first, as with guard-row item (2): enumerate every section of every real response that carries model-authored text, and check which declare the marker. The fix is probably to invert discovery — derive it from `ai_fields` declarations, which already exist and are already tested, rather than from a separate boolean — but that is a design question, not a patch, and it needs its own doc. | None formal; before the boundary is relied on as a release gate. |
...
| **The per-row export payload mixes two computation bases.** `treasury_forecast`, `payouts` and `payout_basis` are recomputed fresh at export; `guardrail` is the one stored at submission time. They agree while the engine is unchanged and diverge exactly when it is not — as on 2026-09-19, when the NPS fix changed the recommendation in 111 of 1,140 cases. **The two overstating names that shared this row are fixed — D-S5, 2026-09-26, `09ef004`..`f8c99fa`, 683 tests OK** (`NEIGHBOURING_ROUTE_CLAIM_SWEEP.md` §7): `summary.total_rows` is now `valid_row_count`, renamed rather than aliased because two names for one value is a second source of truth; and the route no longer claims a current-vs-corrected workbook. The dead `current` argument went with it, because it was what made the false claim look sourced at the call site. | **Low today.** The mixed basis needs a pre-fix row still awaiting export to bite. | `app.py`. Decision D-S4 (label vs. recompute): project owner. | **Label, not recompute** — recomputing silently replaces the verdict a human approved, which erodes the maker-checker gate as a side effect. Recorded as the least certain call in the sweep, and still awaiting this project's owner: a ruling relayed from another session's user is not authorisation here. | None formal. |

...
CLAIMS: tuple = (
    ...
    Claim(
        ...
        threshold_origin="STATUTORY -- s. 124(1)(b) sets 10%, s. 124(2) reads it as "
                         ...
                         "2026-09-14.",
        known_divergence=(
            "TWO DELIBERATE DEPARTURES FROM s. 124, BOTH FROM WHAT THIS TOOL MODELS. "
            "(1) EMPLOYER TYPE: s. 124(1)(a) sets the cap at 14% for a Central or State "
            "Government employer under BOTH regimes. This tool models no employer type and "
            "applies 10% under the old regime to every employer (s. 124(1)(b)). Exact for "
            "non-government employers. "
            "(2) BASE: the statute's base is salary including dearness allowance where the "
            "terms of employment so provide (s. 124(13)(b)). This tool applies the cap to "
            "basic, having no DA field. Exact where DA is zero. "
            "CONSEQUENCES READ FROM THE CODE: tax_engine.derive_nps() sets employer NPS AT "
            "the tool's cap, so for a government employee under the old regime it recommends "
            "less than the law permits; and the 80ccd2_cap guardrail would flag a lawful "
            "10-14% government-employer contribution as exceeding the cap. "
            "DIRECTION FOR TAX: OVER-states, never under-states. Stated 2026-09-19, after the "
            "employer-NPS double count was fixed (TAX_ENGINE_EMPLOYER_NPS_DESIGN.md, commit "
            "02d05a8); until then it was recorded as pending rather than guessed. Why: both "
            "departures make the tool's cap no larger than the lawful one (basic is at most "
            "basic plus DA; 10% is less than 14%), so the tool deducts no more than the law "
            "allows, taxable income is never lower than the lawful figure, and tax does not "
            "fall as taxable income rises. Measured on the fixed engine: 886 structures "
            "(6 CTCs, basic 50/60%, DA 0-50% of basic, government or not, both regimes, four "
            "contribution levels, two rents): 186 over-state, 700 equal, 0 under-state; the "
            "largest over-statement is Rs 1,02,336 (Rs 80L CTC, DA 30% of basic, government "
            "employer, old regime). Exact where the employer is non-government and DA is zero. "
            "Conservative for tax, not for the guardrail, which still flags lawful "
            "contributions (above). "
            "WHAT A REVIEWER IS ACCEPTING is not that the 10%/14% values match the Act -- they "
            "do, for a non-government employer paying no DA -- but that treating every "

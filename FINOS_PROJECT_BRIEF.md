...
685 tests across 23 files, passing with no `ANTHROPIC_API_KEY` set
...
are generated by `scripts/generate_test_counts_md.py` and checked by
`tests/test_doc_test_counts.py`, so they cannot drift from the suite or

...
RULES: tuple = (
    ...
    Rule(
        ...
        rationale="An employer NPS contribution is present but this employee is not recorded as having opted into NPS. The tax computation uses the contribution regardless of the opt-in flag. Within the NPS cap this does not change the tax figure. Above the cap, if the employee has not in fact opted in, the tax shown may be overstated. Confirm the opt-in status before relying on it.",
        why="The two inputs contradict each other. REDRAFTED 2026-09-19 with the employer-NPS tax fix (TAX_ENGINE_EMPLOYER_NPS_DESIGN.md D1-3); the original draft said the contradiction produced a lower tax number, which was the double count itself and is no longer true. tax_engine.taxable_income_for_structure() now adds structure.employer_nps to salary (Income-tax Act, 2025 s. 16(k)) and deducts it up to the cap (s. 124), in both regimes, without ever reading nps_opted. So within the cap the pair has no effect on tax at all. Above the cap the excess is taxed; if the not-opted flag is the true input, that contribution does not exist and the tax is over-stated, never under-stated. The rule flags the inconsistency for confirmation and does not assert which of the two inputs is wrong. Its severity (High) was set when the stated consequence was an under-stated tax; the consequence is now narrower and conservative, and whether High still fits is left to the reviewer rather than changed here",
        ...
        claim_type=CONVENTION,
        basis="Derived entirely from this repository's own code, and checkable by reading it. (1) tax_engine.taxable_income_for_structure() adds structure.employer_nps to gross salary and deducts min(employer_nps, cap x basic), in both the old- and new-regime branches, and nps_opted appears nowhere in that function -- so the contribution is counted whether or not the employee opted in. Within the cap the addition and the deduction cancel; above it the excess stays in taxable income (TAX_ENGINE_EMPLOYER_NPS_DESIGN.md, Option B, commit 02d05a8). Before that fix this claim read `... - structure.employer_nps`, uncapped and never added, which is what made the original rationale's 'understated' true. (2) This pair is UNREACHABLE through the tool's own builder: tax_engine.derive_nps() returns 0.0 when opted_in is false, so build_structure() can never produce it. (3) It IS reachable through /api/batch-audit, which reads `employer_nps` and `nps_opted` as independent CSV columns with independent defaults and performs no cross-field validation. That combination -- impossible internally, reachable externally, and consequential when it occurs -- is the whole basis. NOT CLAIMED, AND DELIBERATELY LEFT FOR THE REVIEWER: whether the employer-contribution deduction is legally available without employee opt-in. That is an interpretation question about the governing provision, this rule does not answer it, and the rationale is worded to flag a contradiction rather than to assert that the tax is wrong in law.",
        status=CANDIDATE,
    ),
    Rule(
        id="R8", severity="High",
